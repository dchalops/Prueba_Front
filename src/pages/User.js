...
const TABLE_HEAD = [
  ...
  { id: 'user_role', label: 'Rol', alignRight: false },
  { id: '' },
];
...
export default function User() {
  ...
  const RegisterSchema = Yup.object().shape({
    username: Yup.string().min(2, 'Demasiado corto!').max(50, 'Demasiado largo!').required('Nombre requerido'),
    ...
    date: Yup.string().required('Fehca requerida'),
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      dispatch(register(values.username, values.email, values.date, values.password));
    },
  });
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, username, email, fecha, role = 'admin', avatarUrl, isVerified, status = 'success' } = row;
